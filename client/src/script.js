...
const walletAddressDOM = document.getElementById("walletAddress");
const txStatusDOM = document.getElementById("tx-status");
const scoreValueDOM = document.getElementById("score-value");
const explorerLinkDOM = document.getElementById("explorer-link");
...
document.getElementById("connectWallet").addEventListener("click", async () => {
  ...
  try {
    ...
    walletAddressDOM.innerHTML = `Connected: ${address.substring(0, 6)}...${address.substring(38)}`;
    walletAddressDOM.classList.add("connected");
...
async function submitScoreToBackend(score) {
  try {
    txStatusDOM.textContent = "Submitting score to blockchain...";
    txStatusDOM.classList.add("submitting");
    showToast("Submitting score to blockchain...", 0);
    ...
    if (json.success) {
      txStatusDOM.textContent = "Score submitted successfully! ✓";
      txStatusDOM.style.color = "#4caf50";
      
      const txHash = json.userOperationHash;
      explorerLinkDOM.href = `https://explorer.monad.xyz/tx/${txHash}`;
      explorerLinkDOM.style.display = "block";
...
document.querySelector("#retry").addEventListener("click", () => {
  ...
  txStatusDOM.textContent = "";
  scoreValueDOM.textContent = "0";
  explorerLinkDOM.style.display = "none";
});
